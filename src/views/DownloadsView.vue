...
/** 正在请求取消的任务 id（避免连点，同时让按钮显示「取消中」）。 */
const cancellingIds = ref<number[]>([]);

/**
 * 取消进行中的任务。
 *
 * 后端会真的停下 —— 包括正在传输的单个大文件（下载循环逐块检查取消标志）。
 * `t.id` 就是后端 `TaskCtx.task_id`。
 */
async function cancelTask(t: TaskEntry) {
  if (cancellingIds.value.includes(t.id)) return;
  ...
  try {
    ...
    cancellingIds.value = cancellingIds.value.filter((id) => id !== t.id);
  }
}
        ...
        <div class="task-top" @click="toggle(t)">
          ...
          <div class="task-side">
            ...
            <!-- 取消：以前这里**没有任何取消入口**，进行中的任务只能等它自己跑完
                 （或者杀掉应用）。`t.id` 就是后端的 TaskCtx.task_id。 -->
            <button
              v-if="!t.finished"
              class="task-cancel"
...
/* 取消按钮：常态就带危险色（触屏没有 hover），命中区按触屏标准给足 */
.task-cancel {
  margin-top: 4px;
  display: inline-flex;
  align-items: center;
