...
<template>
  <div class="browse">
    <!--
      手机横屏是「宽而矮」：类型标签原来独占顶部一整行（43px），挪成左侧竖栏后
      这 43px 全部给结果区（83px → 约 150px）。
      桌面端由 CSS 的 display: contents 还原成原来的纵向排布，不受影响。
    -->
...
/* 横屏手机上这一页的头部（类型卡 + 工具栏 + 筛选标签）会吃掉大量 263px 可用高度。
   实测工具栏里 7 个控件的总宽是 873px，而工具栏只有 829px —— 硬挤成一行会让搜索框
   被压到 150px、占位符被截断、还紧贴右边的控件，非常难看。
   所以这里选「两行但每行都舒服」：行1 约 570px、行2 约 548px，各自单行即可。
   想彻底腾出高度，需要把排序/页大小/视图/来源挪成左侧竖栏（见文档待办）。 */
...
/* ── 类型标签：桌面端保持原来的纵向排布 ─────────────────────────────
   两个包裹层在宽屏下用 display: contents「消失」，
   里面的元素直接成为 .browse 的子元素，和改动前完全一致。 */
.browse-side,
.browse-main {
  display: contents;
}
...
/* ── 手机横屏：类型变左侧竖栏 ──────────────────────────────────────
   横屏 873×393：横向富余、纵向紧张。类型从顶部一行挪到左侧后，
   顶部只剩「搜索 + 实例」和「排序/每页/视图/筛选/来源」两行，
   各自都放得下（约 570px / 548px），不再互相挤压；
   结果区从约 83px 回到约 150px。 */
@media (max-width: 1100px), (pointer: coarse) {
  .browse {
    flex-direction: row;
    gap: 10px;
  ...
  .browse-side {
    display: block;
    /* 与实例详情的 tab 栏共用 --rail-w：两处都出现侧栏时左边缘才对得齐 */
    flex: 0 0 var(--rail-w);
    min-width: 0;
  }
  .browse-main {
  ...
  .browse-side .type-card button {
    text-align: left;
    padding: 7px 10px;
    font-size: 12.5px;
    border-radius: 8px;
  }
  /* 竖栏里不需要横向滑动指示器（用 active 高亮即可） */
  .browse-side .type-card .indicator {
    display: none;
  }
}

/* ── 手机：顶部那整块**不冻结** ────────────────────────────────────
   你要求的：搜索框/关联实例/下载量这一大块不要钉在上面，往上滑要能滚走，
   把高度全部让给内容卡片。
   做法是让「右列整体」滚动（而不是只让结果区滚动）：
   结果区改成随内容自然增高、不再自己滚。 */
@media (max-width: 1100px), (pointer: coarse) {
  .browse-main {
    overflow-y: auto;
