 ...
 * 删除皮肤。
 *
 * 以前是**直接删**：而卡片上的删除按钮默认 `opacity: 0`、只在 hover 时显示 ——
 * 触屏上没有 hover，按钮却仍然参与命中测试，于是「在卡片角上点一下就静默删掉了」。
 * 不可撤销的操作现在一律先确认。
 */
...
async function doDeleteSkin(s: SkinEntry) {
  try {
...
.cape-dialog {
  width: 520px;
  max-width: 90vw;
  /* 披风弹窗在 <Teleport to="body"> 里，不参与 #app 的 zoom */
  max-height: 80vh;
  border-radius: 12px;
  border: 1px solid var(--border);
  background: var(--bg-2);
...
/* ── 手机（含横屏）：从「双栏定高」改成「上下排 + 各自可滚」 ──────────────
   这个视图原本**一个移动断点都没有**，默认布局是照桌面设计的：
     · `.skin-body` 是 `360px 1fr` 双栏 + `min-height: 540px`
     · `.preview-stage` 是 `flex: 1` + `min-height: 320px`
     · `.tabs-pane` 是 `min-height: 540px` + `overflow: hidden`
   横屏手机上内容区只有约 240px 高 → 预览把高度全吃掉、操作按钮全在折叠线以下；
   右侧标签页则被「定高 + 裁切」切掉而且滚不到（naive-ui 的 tabs 自己也是 overflow:hidden）。
   另外 `.skin-canvas` 的 `touch-action: none` 会在手指落在画布上时**吞掉滚动起手区**，
   而画布几乎占满预览区 —— 于是竖滑也走不动。

   判据沿用全项目既有的「三条件取或」（部分 WebView 会误报 pointer: fine）。 */
@media (max-width: 1100px), (pointer: coarse) {
  .skin-body {
    grid-template-columns: minmax(0, 1fr);
    /* 覆盖基类的 min-height: 540px：横向空间不够就上下排 */
    min-height: 0;
    gap: 12px;
  }
  .preview-pane {
  ...
  .preview-stage {
    /* 不再当「吃掉剩余高度」的定高块用，给一个手机上合适的预览高 */
    flex: none;
    min-height: 180px;
    height: 180px;
    /* 限宽 + 居中：单列之后预览会撑满整行，横屏下变成 796×180 的横幅 ——
       皮肤模型按高缩放，在那种比例里会缩成很小一个点。 */
    width: 100%;
    max-width: 260px;
    margin: 0 auto;
  }
  /* 标签页：定高 + overflow:hidden 在矮内容区里等于把列表切掉且滚不到 */
  .tabs-pane {
    min-height: 0;
    overflow: visible;
    padding: 12px;
  ...
  /* ── 宽而矮的横屏手机：保持**左右两栏**（预览 | 皮肤列表）─────────────
     原实现手机上一律改成上下排，原因是「预览撑满整行会变成 796×180 的横幅、
     模型按高缩放后缩成一点」—— 但正确解法是**收窄预览栏**而不是堆成一列：
     853×384 这类视口横向富余，两栏后模型有 180px 高可撑、右侧列表也有完整行高。
     用宽度而非 orientation 判据：竖屏手机（384px 宽）仍走上下的单列布局。 */
  @media (min-width: 620px) {
    ...
    .preview-stage {
      ...
      max-width: none;
      margin: 0;
      height: clamp(140px, 42vh, 200px);
      min-height: 0;
    }
  }

  /* 画布把纵向手势还给浏览器：竖滑＝滚页面，横滑＝旋转模型。
     旋转另有显式的「自动旋转」开关兜底（见 .rotate-row），不依赖拖拽。

     **必须 `!important`**：监听画布的 3D 库会把 `touch-action: none` 写成
     **行内样式**（实测画布上有 `style="touch-action: none; width: 325px; height: 179px"`，
     还带 `data-engine` 属性），行内优先级高于类选择器 —— 只用类选择器改不动它。
     竖滑被浏览器接管后，库会收到 `pointercancel` 正常收尾，不会卡住拖拽。 */
  .skin-canvas {
