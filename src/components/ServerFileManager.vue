...
<template>
  <div class="fm" :class="{ 'fm-editing': !!active }">
    ...
    <!-- 确认弹窗。Teleport 到 body 的原因见 FileManager 同名注释：
...
/* ── 手机横屏：改成单栏（与 FileManager.vue 同一套策略）───────────────
   固定 400px 左栏在横屏手机上会让「文件名 + 行内操作按钮」挤在一起被 overflow 裁掉；
   `min-height: 420px` 更是超过整个视口（横屏约 393 CSS px）。 */
.fm-back {
  display: none;
  align-items: center;
  justify-content: center;
