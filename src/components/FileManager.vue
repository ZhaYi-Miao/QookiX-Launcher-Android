...
<template>
  <div class="fm" :class="{ 'fm-editing': !!active }">
    <!-- 左侧：文件浏览 -->
    ...
    <!-- 确认弹窗。
         必须 Teleport 到 body：`.app-bg > *` 给 `.body` 加了 z-index（见 styles.css），
         于是 `.body` 成了层叠上下文 —— 写在它内部的 position:fixed 覆盖层无论
         z-index 多高都只跟 `.body` 内部比较，而 `.mobile-nav` 是 `.body` 的**兄弟节点**，
         在 `.app-bg` 这一层解析，结果恒在遮罩之上（底栏不变暗且仍可点）。 -->
    <Teleport to="body">
...
.fm-mask {
  position: fixed;
  inset: 0;
  /* 1200：要压过 `.mobile-nav` 底栏与 ContextMenu（同值靠 DOM 顺序，Teleport 在后面） */
  z-index: 1200;
...
/* ── 手机横屏：改成单栏 ───────────────────────────────────────────────
   两个问题一起修：
   1. 原来是「400px 固定左栏 + 自适应右栏」。手机横屏宽约 873 CSS px，
      左栏 400px 里「图标 + 文件名 + 三个行内操作按钮」放不下，
      行内按钮被 overflow: hidden 裁掉 —— 这就是「操作按钮看不到」的真因。
   2. `height: calc(100vh - 320px)` + `min-height: 420px`：横屏手机整个视口才约 393px 高，
      420px 的最小高度直接超出屏幕，内容和底部导航互相压。
   改成：一次只显示一栏（列表 / 编辑器），高度按可用空间算、不再设超大 min-height。 */
.fm-back {
  display: none;
  align-items: center;
  justify-content: center;
...
@media (max-width: 1100px), (pointer: coarse) {
  .fm {
    flex-direction: column;
    /* 实测：横屏手机视口约 393 CSS px，标题栏 + 实例头部 + 9 个标签 + 底部导航
       已经占掉约 307px，剩下的才是列表能用的高度。这里给 310 的余量，
       min-height 必须是 0 —— 原来的 420px 比整个视口还高，内容会压到底部导航上。 */
    /* 用 flex 填满父容器即可：页面级滚动已取消，.d-body/.tab-body 都有确定高度。
       （原来是 height: 52vh 的估算值 —— 那时父容器没有确定高度，只能靠猜。） */
    height: 100%;
    flex: 1;
    min-height: 0;
  }
  ...
  .fm.fm-editing .fm-side {
    display: none;
  }
  .fm.fm-editing .fm-main {
    display: flex;
    flex: 1;
    min-height: 0;
  }
