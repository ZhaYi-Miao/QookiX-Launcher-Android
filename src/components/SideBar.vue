...
<template>
  <aside ref="sidebarRef" class="sidebar" :class="{ collapsed }">
    ...
    <div class="side-foot">
      ...
      <AccountChip v-if="!isMobile" :collapsed="collapsed" />
      <button
...
.sidebar {
  position: relative;
  /* 一级导航：比二级侧栏（`--rail-w`）宽一档，因为要放「游戏实例」这类 4 字标签 +
     图标 + 计数徽标。同样不写死：窄屏略收、宽屏略放。

     用 vw 是必须除 --ui-scale 的：`#app` 上有 zoom，而 **vw 不会被 zoom 折算**
     （实测 150% 时 `100vw` 的探针量到 1280px = 853×1.5）。
     上下限保持布局像素不折算，理由同 styles.css 里 `--rail-w` 的注释。 */
  width: clamp(188px, calc(24vw / var(--ui-scale, 1)), 236px);
  flex-shrink: 0;
  display: flex;
