...
// ── 兜底：`launch://crash` 后端从来没 emit 过 ─────────────────────────────
// 全仓搜 `launch://crash` 只有上面这一处「消费」、零处「发送」，
// 所以崩溃弹窗在手机上永远不弹。这里退一步直接盯 `launch://state` 的**非正常退出**
// —— 那个事件本来就带 exit code（见 stores/tasks.ts 的 lastExit），够弹一个
// 「游戏异常退出 + 去看日志」的提示了。
//
// 下面这些退出码是**用户主动终止**（从抽屉「强制关闭」、系统杀后台、滑掉任务卡），
// 必须排除，否则会出现「我明明自己关的，它还说我崩了」：
//   130 = SIGINT  137 = SIGKILL  143 = SIGTERM
const USER_KILL_CODES = [130, 137, 143];
let lastHandledExit = "";
...
watch(
  ...
  (exit) => {
    ...
    if (key === lastHandledExit) return;
    const code = exit.code;
    if (code == null || code === 0 || USER_KILL_CODES.includes(code)) return;
    lastHandledExit = key;
    info.value = {
