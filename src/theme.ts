...
export function buildDarkOverrides(accent: string): GlobalThemeOverrides {
  ...
  return {
    ...
    Menu: {
      // 左侧竖向导航栏（实例详情的选项卡）。手机横屏纵向紧张，把每项压到 26px。
      //
      // 注意：这里**不是**「9 × 26 ≈ 234px 就放得下」。实际占用还要加上
      // `.d-rail` 自身的 padding/gap（≈24px）和它下面的 `.d-rail-actions` 整块（≈73px），
      // 最坏约 320px，而横屏手机可用高度只有约 240px —— 所以 `.d-rail` 必须允许自己滚动
      // （已在 InstanceDetailView.vue 加 `overflow-y: auto; min-height: 0`），
      // 光靠压低 itemHeight 是解决不了的。
      itemHeight: "26px",
      borderRadius: "8px",
      itemTextColor: "#d4d5dd",
      itemTextColorHover: "#f2f3f7",
      itemTextColorActive: primary,
      itemTextColorActiveHover: primary,
      itemIconColor: "#9b9daa",
      itemIconColorHover: "#f2f3f7",
      itemIconColorActive: primary,
      itemIconColorActiveHover: primary,
      itemColorHover: "rgba(255,255,255,0.06)",
      itemColorActive: rgba(accent, 0.14),
      itemColorActiveHover: rgba(accent, 0.2),
    },    Button: {
...
export function buildLightOverrides(accent: string): GlobalThemeOverrides {
  ...
  return {
    ...
    Menu: {
      ...
      itemHeight: "26px",
      borderRadius: "8px",
      itemTextColor: "#4a4d58",
      itemTextColorHover: "#1a1d24",
      itemTextColorActive: primary,
      itemTextColorActiveHover: primary,
      itemIconColor: "#8b8e9c",
